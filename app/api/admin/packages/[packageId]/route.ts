...
import { deleteRecipePackage, updateRecipePackage } from "@/lib/server/admin";
...
export async function DELETE(
  ...
  try {
    ...
    const { packageId } = await params;
    const recipePackage = await deleteRecipePackage(packageId);
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to delete recipe package." },
      { status: 400 },
    );
  }
}
